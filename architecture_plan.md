...
This document outlines the architecture for a scalable, event-driven speech processing platform designed to run on Azure
Kubernetes Service (AKS). The system is engineered to handle high-volume audio ingestion, intelligent routing, and
parallel processing using both local GPU resources and Azure AI services.
...
- **Hybrid Processing:** Utilization of cost-effective local models (Whisper V3 on GPU) alongside high-fidelity Azure AI
  services.
...
To facilitate parallel development by different teams (Platform, Backend, ML), the codebase is split into three logical
repositories:
...
01. **Submit:** Client `POST /jobs` → API creates job record, returns SAS URL
02. **Upload:** Client uploads audio directly to Blob Storage using SAS URL
03. **Start:** Client `POST /jobs/{id}/start` → API sends message to `job-events` queue
04. **Route:** Router consumes message, creates LID step, sends to `lid-jobs` queue
05. **LID:** LID Worker processes audio, detects language, updates DB, sends completion event
06. **Route:** Router sees LID complete, creates TRANSCRIBE step, routes to `whisper-jobs`
07. **Transcribe:** Whisper Worker transcribes audio (GPU), saves to blob, updates DB
08. **Route:** Router sees TRANSCRIBE complete, routes to Azure AI for translation
09. **Translate:** Azure AI Worker translates (if non-English), saves result
