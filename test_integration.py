...
class IntegrationTest:
    ...
    def _generate_test_audio(self, duration_seconds: float = 1.0, sample_rate: int = 16000) -> bytes:
        ...
        for i in range(num_samples):
            sample = int(32767 * 0.3 * struct.unpack('d', struct.pack('d', 
                2.0 * 3.14159 * frequency * i / sample_rate))[0])
    ...
    def test_04_start_job(self) -> bool:
        ...
        try:
            response = requests.post(
                f"{self.api_base_url}/jobs/{self.job_id}/start",
                timeout=10
            )
    ...
    def test_05_get_job_status(self) -> bool:
        ...
        try:
            response = requests.get(
                f"{self.api_base_url}/jobs/{self.job_id}",
                timeout=10
            )
            ...
            # Validate response structure
            required_fields = ["job_id", "status", "workflow_type", "audio_filename", 
    ...
    def test_06_list_jobs(self) -> bool:
        ...
        try:
            # Test basic list
            response = requests.get(
                f"{self.api_base_url}/jobs?limit=10",
                timeout=10
            )
    ...
    def test_07_workflow_validation(self) -> bool:
        ...
        try:
            ...
            response = requests.get(
                f"{self.api_base_url}/jobs/{fake_job_id}",
                timeout=10
            )
