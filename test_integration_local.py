...
def test_create_job(workflow_type: str = "transcribe_only", 
                    source_language: str = "en") -> Optional[Dict[str, Any]]:
...
def test_get_job_results(job_id: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        response = requests.get(
            f"{API_BASE_URL}/jobs/{job_id}/results",
            timeout=10
        )
...
def test_invalid_job_id() -> bool:
    ...
    try:
        response = requests.get(
            f"{API_BASE_URL}/jobs/{fake_job_id}",
            timeout=10
        )
...
def test_start_job_without_upload(job_id: str) -> bool:
    ...
    payload = {
        "audio_filename": "never-uploaded.wav",
    ...
    try:
        ...
        start_response = requests.post(
            f"{API_BASE_URL}/jobs/{new_job_id}/start",
            timeout=10
        )
...
def print_final_results(results: Dict[str, int]):
    ...
    print("║" + " " * 29 + status + " " * (49 - len(status.replace(Colors.OKGREEN, '').replace(Colors.FAIL, '').replace(Colors.WARNING, '').replace(Colors.OKBLUE, ''))) + "║")
    print("╚" + "═" * 78 + "╝")
    print(f"{Colors.ENDC}\n")
