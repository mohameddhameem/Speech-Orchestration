...
app = FastAPI(
    title="Speech Flow API",
    description="Event-driven speech processing pipeline",
    version="1.0.0"
)
...
class JobSubmitRequest(BaseModel):
    ...
    workflow_type: Literal[
        ...
        "translate_only",
        "summarize_only"
    ] = Field(default="full_pipeline", description="Type of processing workflow")
...
async def get_job_results(job_id: str, db: Session = Depends(get_db)):
    ...
    if "LID" in completed_steps:
        lid_result = read_blob_json(
            settings.RESULTS_CONTAINER_NAME,
            f"{job_id}/lid_result.json"
        )
        if lid_result:
    ...
    if "TRANSCRIBE" in completed_steps:
        transcription_result = read_blob_json(
            settings.RESULTS_CONTAINER_NAME,
            f"{job_id}/transcription.json"
        )
    ...
    if "TRANSLATE" in completed_steps:
        translation_result = read_blob_json(
            settings.RESULTS_CONTAINER_NAME,
            f"{job_id}/translation.json"
        )
    ...
    if completed_steps:
        last_completed = max(
            (s.completed_at for s in completed_steps.values() if s.completed_at),
            default=None
        )
