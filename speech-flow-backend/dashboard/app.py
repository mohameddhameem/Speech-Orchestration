...
def get_queue_depths():
    ...
    try:
        ...
        for q in queues:
            ...
            data.append(
                {"Queue": q, "Active": props.active_message_count, "Dead Letter": props.dead_letter_message_count}
            )
...
date_range = st.sidebar.selectbox("Select Range", ["Last 24 Hours", "Last 7 Days", "Last 30 Days"], index=0)
...
with col3:
    ...
    st.metric(
        "Failed",
        f"{failed:,}",
        delta=f"-{(failed/total_jobs*100):.1f}%" if total_jobs > 0 else "0%",
        delta_color="inverse",
    )
...
with col1:
    ...
    if not step_df.empty:
        ...
        # Show detailed table
        st.dataframe(
            step_df[["step_name", "count", "avg_queue_wait_ms", "avg_processing_ms", "avg_rtf", "total_tokens"]].round(
                2
            ),
            hide_index=True,
        )
...
with col2:
    ...
    if not daily_df.empty:
        fig = px.line(
            daily_df, x="metric_date", y="total_cost_usd", labels={"metric_date": "Date", "total_cost_usd": "Cost ($)"}
        )
        fig.update_layout(height=300)
        st.plotly_chart(fig, use_container_width=True)
    else:
...
if not jobs_df.empty:
    ...
    # Add status emoji
    status_emoji = {"COMPLETED": "✅", "FAILED": "❌", "PROCESSING": "🔄", "PENDING": "⏳", "QUEUED": "📥"}
