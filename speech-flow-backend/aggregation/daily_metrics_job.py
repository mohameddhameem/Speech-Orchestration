...
from models import Job, JobStep, DailyMetrics
...
def aggregate_daily_metrics(db: Session, target_date: date) -> DailyMetrics:
    ...
    steps = db.query(JobStep).filter(
        JobStep.queued_at >= start_time,
        JobStep.queued_at < end_time
    ).all()
    ...
    transcription_rtfs = [
        float(s.transcription_rtf) for s in steps 
        if s.step_name == "TRANSCRIBE" and s.transcription_rtf is not None
    ]
    
    # LID confidence (only from LID steps)
    lid_confidences = [
        float(s.language_confidence) for s in steps
        if s.step_name == "LID" and s.language_confidence is not None
    ]
    ...
    metrics.avg_transcription_rtf = (
        Decimal(str(round(sum(transcription_rtfs) / len(transcription_rtfs), 3)))
        if transcription_rtfs else None
    )
    metrics.avg_lid_confidence = (
        Decimal(str(round(sum(lid_confidences) / len(lid_confidences), 4)))
        if lid_confidences else None
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int, 
        default=1,
        help="Number of days to aggregate (default: 1 for yesterday)"
    )
    parser.add_argument(
        "--date",
        type=str,
